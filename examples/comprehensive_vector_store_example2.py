...
# Safe print function to handle encoding errors when redirecting output
def safe_print(*args, **kwargs):
    """Print function that handles encoding errors gracefully."""
    try:
        print(*args, **kwargs)
    except (UnicodeEncodeError, OSError):
        # If encoding fails, try to print without problematic characters
        try:
...
# Check if running in non-interactive mode (output redirected, stdin not available)
def is_non_interactive():
    """Check if running in non-interactive mode (output redirected or stdin unavailable)."""
    import sys
    try:
...
# Safe input function that provides defaults in non-interactive mode
def safe_input(prompt, default="", non_interactive_default=""):
    """Input function that handles non-interactive mode gracefully."""
    if is_non_interactive():
        if non_interactive_default:
            safe_print(f"{prompt} [non-interactive: using '{non_interactive_default}']")
            return non_interactive_default
        elif default:
            safe_print(f"{prompt} [non-interactive: using '{default}']")
            return default
        else:
            safe_print(f"{prompt} [non-interactive: skipping]")
            return ""
...
class UnifiedVectorStoreExample:
    ...
    def generate_embeddings_in_batches(self, tokens=None, embedding_gen=None, batch_size=50000):
        ...
        for batch_idx in range(num_batches):
            ...
            print(f"\n[INFO] Processing batch {batch_idx + 1}/{num_batches} (tokens {start_idx:,} to {end_idx:,})...")
    ...
    def load_vector_store_from_disk(self, max_batches: int = 30):
        ...
        if hasattr(self, '_loading_in_progress') and self._loading_in_progress:
            print("[INFO] Loading already in progress, skipping duplicate call...")
            return True  # Assume success if already loading
        ...
        try:
            ...
            safe_print(f"[INFO] Loading tokens from {tokens_file}...")
            try:
            ...
            print(f"[INFO] Loading {len(batch_files)} batches into vector stores...")
            print("[INFO] Using memory-safe loading with error recovery...")
            
            total_added = 0
            faiss_failed = False
            chroma_failed = False
            weaviate_failed = False
            faiss_total = 0
            chroma_total = 0
            weaviate_total = 0
            ...
            for batch_idx, batch_file in enumerate(batch_files):
                ...
                try:
                    ...
                    # Add to Weaviate (most reliable for large datasets, but can have memory limits)
                    if self.weaviate_store and not weaviate_failed:
                        try:
                            ...
                            weaviate_total += len(batch_tokens)
                        except Exception as e:
                            error_msg = str(e).lower()
                            # Check for various error types that indicate Weaviate is unavailable
                            is_critical_error = (
                                'memory' in error_msg or 
                                'not enough' in error_msg or 
                                'disk usage' in error_msg or 
                                'read-only' in error_msg or
                                'unavailable' in error_msg or
                                'no healthy upstream' in error_msg or
                                'statuscode.unavailable' in error_msg or
                                'grpc_status:14' in error_msg or
                                'failed after' in error_msg  # "failed after 3 retries"
                            )
                            
                            if is_critical_error:
                                print(f"  [WARNING] Weaviate critical error at batch {batch_idx + 1}: {e}")
                                print(f"  [INFO] Weaviate will be skipped for remaining batches.")
                                weaviate_failed = True
                                self.weaviate_store = None
                            else:
                    ...
                    if (batch_idx + 1) % 10 == 0:
                        ...
                        if not weaviate_failed and self.weaviate_store:
                            status.append(f"Weaviate: {weaviate_total:,}")
                        status_str = ", ".join(status) if status else "All stores: skipped/failed"
            ...
            if not weaviate_failed and self.weaviate_store:
                print(f"  Weaviate: {weaviate_total:,} tokens loaded")
            elif weaviate_failed:
                print(f"  Weaviate: Failed (memory/disk limit) - skipped")
            else:
                print(f"  Weaviate: Not available")
            
            # IMPORTANT: Return True even if some vector stores failed
            # because tokens and embeddings are successfully loaded in memory
            # The user can still use FAISS or work with in-memory data
            print(f"\n[INFO] Tokens and embeddings are loaded in memory and ready to use!")
            if faiss_failed and chroma_failed and weaviate_failed:
                print(f"[WARNING] All vector stores failed, but you can still:")
                print(f"  • Use in-memory embeddings for similarity calculations")
                print(f"  • Generate new embeddings")
                print(f"  • Try loading fewer batches (reduce max_batches parameter)")
    ...
    def initialize_vector_stores(self, use_weaviate=True, use_faiss=True, use_chroma=True):
        ...
        # Weaviate (with timeout to prevent hanging)
        if use_weaviate and WEAVIATE_AVAILABLE:
            try:
                print("\n[INFO] Initializing Weaviate...")
                print("[INFO] This may take a moment (connecting to cloud server)...")
                
                # Use threading to add timeout for Weaviate connection
                import threading
                import queue
                ...
                def init_weaviate():
                    try:
                        ...
                        result_queue.put(store)
                    except Exception as e:
                        exception_queue.put(e)
                
                # Start initialization in a separate thread
                thread = threading.Thread(target=init_weaviate, daemon=True)
                thread.start()
                thread.join(timeout=30)  # 30 second timeout
                
                if thread.is_alive():
                    # Thread is still running - connection is hanging
                    print("[WARNING] Weaviate connection timeout (30s). Skipping Weaviate.")
                    print("[INFO] Continuing with FAISS and ChromaDB only.")
                    self.weaviate_store = None
                    ...
                    # Success
                    self.weaviate_store = result_queue.get()
                    print("[OK] Weaviate initialized!")
                else:
                    # Shouldn't happen, but handle it
                    print("[WARNING] Weaviate initialization returned no result. Skipping.")
                    self.weaviate_store = None
    ...
    def store_tokens_weaviate_first(self, tags: Optional[Dict] = None):
        """Store tokens and embeddings in Weaviate FIRST (priority - cloud-based, reliable)."""
        print("\n" + "=" * 80)
        print("STORING TOKENS IN WEAVIATE (PRIORITY)")
        print("=" * 80)
        ...
        # Store in Weaviate FIRST (priority - most reliable)
        if self.weaviate_store:
            try:
                print("\n[INFO] Storing in Weaviate (cloud-based, reliable)...")
                print("[INFO] Deduplication enabled: skipping tokens that already exist")
                self.weaviate_store.add_tokens(
                    self.tokens, 
                    self.embeddings, 
                    metadata=metadata_list,
                    skip_duplicates=True  # Prevent duplicates - same token won't be stored twice
                )
                print(f"[OK] Stored tokens in Weaviate (duplicates automatically skipped)")
                # Store tags for later use
                self._last_tags = tags
                return True
            except Exception as e:
                error_msg = str(e).lower()
                if 'read-only' in error_msg or 'disk usage' in error_msg or 'too high' in error_msg:
                    print(f"[WARNING] Weaviate is read-only (disk usage too high). Skipping Weaviate storage.")
                    print(f"  → Error: {e}")
                    # Mark Weaviate as unavailable for this session
                    self.weaviate_store = None
                else:
    ...
    def store_tokens_other_stores(self, tags: Optional[Dict] = None):
        """Store tokens in FAISS and ChromaDB (at the end, after Weaviate is done)."""
        print("\n" + "=" * 80)
        print("STORING TOKENS IN OTHER VECTOR STORES (FAISS & ChromaDB)")
        print("=" * 80)
        
        # Use last tags if available
        if tags is None:
            tags = getattr(self, '_last_tags', None)
        ...
        if self.faiss_store:
            try:
                ...
                print(f"[WARNING] FAISS out of memory. Skipping FAISS storage.")
            except Exception as e:
        ...
        if self.chroma_store:
            try:
                ...
                error_msg = str(e).lower()
                if 'compaction' in error_msg or 'hnsw' in error_msg:
                    print(f"[WARNING] ChromaDB compaction error. Skipping ChromaDB storage.")
                else:
                    print(f"[WARNING] Failed to store in ChromaDB: {e}")
    
    def store_tokens(self, tags: Optional[Dict] = None, weaviate_first: bool = True):
        """
        Store tokens and embeddings in vector stores.
        
        Args:
            tags: Optional metadata tags
            weaviate_first: If True, store in Weaviate first, then other stores. 
                          If False, store in all stores at once (old behavior).
        """
        if weaviate_first:
            # New behavior: Weaviate first
            self.store_tokens_weaviate_first(tags)
        else:
            # Old behavior: All stores at once
            self.store_tokens_weaviate_first(tags)
            self.store_tokens_other_stores(tags)
    
    def _get_token_embedding(self, token, token_idx: Optional[int] = None) -> Optional[np.ndarray]:
        """
        Safely get embedding for a token.
        Handles cases where embeddings might not be loaded for all tokens.
        """
        # Try to get from loaded embeddings first (if within bounds)
        if token_idx is not None and self.embeddings is not None:
            if token_idx < len(self.embeddings):
                return self.embeddings[token_idx].copy()
        
        # If not in loaded embeddings, generate embedding on the fly
        # (Vector stores don't have text-based search, only vector-based)
        try:
            token_text = getattr(token, 'text', '')
            if token_text:
                query_streams = self.tokenizer.build(token_text)
                query_tokens = []
            ...
            # Silently fail - will return None
            pass
    ...
    def semantic_search(self, query_text: str, top_k: int = 10, store_name: str = "all", 
        ...
        for token in self.tokens:
            if getattr(token, 'text', '').lower() == query_text.lower():
                ...
                query_embedding = self._get_token_embedding(token, token_idx)
        ...
        for token_text in concept_tokens:
            for token in self.tokens:
                if getattr(token, 'text', '').lower() == token_text.lower():
                    token_idx = self.tokens.index(token)
                    emb = self._get_token_embedding(token, token_idx)
                    if emb is not None:
    ...
    def compare_tokens(self, token1: str, token2: str):
        ...
        # Try to get embeddings from vector store first (fastest)
        emb1 = None
        emb2 = None
        ...
        if self.weaviate_store:
            try:
                # Search for token1
                query_streams1 = self.tokenizer.tokenize(token1)
                if query_streams1:
                    query_tokens1 = []
                    for stream_name, token_stream in query_streams1.items():
                        query_tokens1.extend(token_stream.tokens)
                    if query_tokens1:
                        query_embeddings1 = self.embedding_gen.generate_batch(query_tokens1)
                        query_emb1 = np.mean(query_embeddings1, axis=0)
                        # Search for similar tokens in Weaviate
                        results1 = self.weaviate_store.search(query_emb1, top_k=1)
                        if results1 and results1[0].get('text', '').lower() == token1.lower():
                            # Found exact match, use query embedding
                            emb1 = query_emb1
                
                # Search for token2
                query_streams2 = self.tokenizer.tokenize(token2)
                if query_streams2:
                    query_tokens2 = []
                    for stream_name, token_stream in query_streams2.items():
                        query_tokens2.extend(token_stream.tokens)
                    if query_tokens2:
                        query_embeddings2 = self.embedding_gen.generate_batch(query_tokens2)
                        query_emb2 = np.mean(query_embeddings2, axis=0)
                        # Search for similar tokens in Weaviate
                        results2 = self.weaviate_store.search(query_emb2, top_k=1)
                        if results2 and results2[0].get('text', '').lower() == token2.lower():
                            # Found exact match, use query embedding
                            emb2 = query_emb2
            except Exception:
                pass
        
        # If not found in vector store, generate embeddings directly (no iteration through all tokens)
        if emb1 is None:
            try:
                query_streams1 = self.tokenizer.tokenize(token1)
                if query_streams1:
                    ...
                    if query_tokens1:
                        query_embeddings1 = self.embedding_gen.generate_batch(query_tokens1)
                        emb1 = np.mean(query_embeddings1, axis=0)
            except Exception as e:
                print(f"[WARNING] Could not generate embedding for '{token1}': {e}")
        ...
        if emb2 is None:
            try:
                ...
                if query_streams2:
                    ...
                    if query_tokens2:
                        query_embeddings2 = self.embedding_gen.generate_batch(query_tokens2)
                        emb2 = np.mean(query_embeddings2, axis=0)
            except Exception as e:
                print(f"[WARNING] Could not generate embedding for '{token2}': {e}")
        
        if emb1 is None or emb2 is None:
            print("[ERROR] One or both tokens could not be embedded")
            if emb1 is None:
                print(f"  → Could not get embedding for '{token1}'")
            if emb2 is None:
                print(f"  → Could not get embedding for '{token2}'")
            return None
    ...
    def find_concept_cluster(self, seed_concept: str, cluster_size: int = 10, min_similarity: float = 0.6):
        ...
        for token in self.tokens:
            if getattr(token, 'text', '').lower() == seed_concept.lower():
                token_idx = self.tokens.index(token)
                seed_emb = self._get_token_embedding(token, token_idx)
                if seed_emb is not None:
                    break
        
        if seed_emb is None:
            print(f"[ERROR] Seed concept '{seed_concept}' not found or embedding unavailable")
            return []
        ...
        if cluster:
            print(f"\n[INFO] Concept Cluster (found {len(cluster)} concepts):")
    ...
    def interactive_search(self):
        ...
        # Skip interactive mode if running non-interactively
        if is_non_interactive():
            print("\n[INFO] Skipping interactive mode (non-interactive session)")
            return
        ...
        while True:
            try:
                try:
                    cmd = safe_input("\n[INFO] > ", default="exit", non_interactive_default="exit").strip().split()
                except (KeyboardInterrupt, EOFError):
                    cmd = ["exit"]
                if not cmd:
                    continue
    ...
    def evaluate_quality(self, probe_tokens: List[str], top_k: int = 10):
        ...
        for probe in probe_tokens:
            ...
            if query_embedding is None:
                print(f"  [WARNING] Probe '{probe}' not found or embedding unavailable")
                continue
    ...
    def evaluate_semantic_alignment(self, test_pairs: Optional[List[Tuple[str, str, float]]] = None):
        ...
        if mean_error > 0.3:
            print(f"\n[WARNING] HIGH MISALIGNMENT: SanTOK embeddings don't match human semantics well.")
            print(f"   Consider training embedding alignment (see train_embedding_alignment)")
        elif mean_error > 0.2:
            print(f"\n[WARNING] MODERATE MISALIGNMENT: Some improvement needed.")
        else:
            print(f"\n[OK] GOOD ALIGNMENT: SanTOK embeddings align well with human semantics.")
    ...
    def build_context_fusion_embeddings(
        ...
        # Check if we have embeddings for all tokens
        if self.embeddings is None or len(self.embeddings) < len(self.tokens):
            print(f"[ERROR] Context fusion requires embeddings for all tokens.")
            print(f"  → Loaded: {len(self.embeddings) if self.embeddings is not None else 0}, Required: {len(self.tokens)}")
            print(f"  → Please generate embeddings for all tokens first.")
            return None
        ...
        for i, token in enumerate(self.tokens):
            if i >= len(self.embeddings):
                ...
                context_embeddings[i] = np.zeros(embedding_dim)
                continue
            ...
            if use_neighbor_attention:
                ...
                def add_neighbor_if_similar(neighbor_idx, weight_multiplier=1.0):
                    if neighbor_idx is None or neighbor_idx < 0 or neighbor_idx >= len(self.tokens):
                        return
                    if neighbor_idx >= len(self.embeddings):
                        return  # Skip if embedding not available
    ...
    def close_stores(self):
        ...
        try:
            ...
            # Handle encoding errors when redirecting output
            pass
...
def main():
    ...
    WORKFLOW (Weaviate-First Strategy):
    ====================================
    
    PHASE 1: INITIALIZATION
    - Check dependencies, ask for embedding strategy
    - Initialize vector stores (Weaviate, FAISS, ChromaDB)
    - Show feature selection menu
    
    PHASE 2: DATA PREPARATION (IMMEDIATE WEAVIATE STORAGE)
    - Tokenize text → Generate embeddings
    - [PRIORITY] STORE IN WEAVIATE FIRST (Line 2720 or 2738)
      → This ensures data is safe in cloud before any other operations
      → If this fails, workflow stops (critical step)
    
    PHASE 3-7: ALL OTHER OPERATIONS
    - Search & Retrieval (d, f)
    - Analysis & Comparison (g, h, i, j, k)
    - Embeddings & Documents (l, m)
    - Evaluation & Quality (n, o)
    - Advanced Features (r, s, p, q, t)
    → All these use Weaviate data (already stored)
    
    PHASE 8: FINAL STORAGE (END OF WORKFLOW)
    - [FINAL] STORE IN FAISS/CHROMADB (Line 2868)
      → Happens at the very end, after all operations
      → If FAISS/ChromaDB fail, no problem - Weaviate has everything
      → This is "bonus" storage for faster local search
    
    BENEFITS:
    - Data safety: Weaviate (cloud) gets data immediately
    - No time wasting: Operations continue even if local stores fail
    - Graceful degradation: Failures don't break the workflow
    """
    print("=" * 80)
    ...
    try:
        ...
        choice = safe_input("\nEnter choice (1 or 2) [default: 1]: ", default="1", non_interactive_default="1")
        if choice == "2":
            use_hybrid = True
    ...
    def show_feature_menu():
        ...
        try:
            choice = safe_input("\nEnter your selection (e.g., 'a,d,g' or 'all') [default: all]: ", default="all", non_interactive_default="all")
            if not choice:
                choice = "all"
            return choice
        except (KeyboardInterrupt, EOFError):
            return "all"  # Default to "all" instead of "exit" in non-interactive mode
    ...
    if needs_data:
        ...
        if resume:
            # Check data size to show user what they have
            try:
                import types
                project_root = Path(__file__).parent.parent
                if str(project_root) not in sys.path:
                ...
                # Quick check of data size
                with open(tokens_file, 'rb') as f:
                    try:
                        unpickler = CustomUnpickler(f)
                        temp_tokens = unpickler.load()
                        token_count = len(temp_tokens)
                    except Exception:
                        f.seek(0)
                        temp_tokens = pickle.load(f)
                        token_count = len(temp_tokens)
                
                # Check embedding batches
                batch_count = 0
                total_embeddings = 0
                if os.path.exists(metadata_file):
                    with open(metadata_file, 'r') as f:
                        ...
                        batch_count = len([f for f in batch_files if os.path.exists(f)])
                        total_embeddings = metadata.get("total_tokens", 0)
                
                print("\n" + "=" * 80)
                print("EXISTING DATA FOUND - YOUR WORK IS SAFE!")
                print("=" * 80)
                print(f"✓ Found {token_count:,} processed tokens")
                if batch_count > 0:
                    print(f"✓ Found {batch_count} embedding batch files")
                    print(f"✓ Total embeddings: {total_embeddings:,}")
                print("\n[SAFETY GUARANTEE]")
                print("  • Your existing data will NOT be overwritten")
                print("  • Resuming will USE your existing data (no re-processing)")
                print("  • New data will be ADDED (not replace existing)")
                print("  • Vector stores use skip_duplicates=True (prevents duplicates)")
                print("\n[INFO] You can resume from where you left off.")
                print("=" * 80)
            except Exception as e:
                print("\n[INFO] Found existing tokens. You can resume from where you left off.")
                print(f"[NOTE] Could not check data size: {e}")
            
            try:
                resume_choice = safe_input("\nResume and use existing data? (y/n) [default: y]: ", default="y", non_interactive_default="y")
                resume = (resume_choice or "y").lower() == "y"
            except (KeyboardInterrupt, EOFError):
            ...
            if resume:
                ...
                if os.path.exists(metadata_file):
                    if example.load_vector_store_from_disk(max_batches=30):
                        print("\n" + "=" * 80)
                        print("✓ DATA SUCCESSFULLY LOADED - YOUR WORK IS PRESERVED!")
                        print("=" * 80)
                        print(f"✓ Loaded {len(example.tokens):,} tokens from disk")
                        print("✓ Loaded embeddings from batch files")
                        print("✓ Skipping tokenization and embedding generation (using existing data)")
                        print("✓ Your existing data is safe and will NOT be overwritten")
                        print("=" * 80)
                        ...
                        try:
                            ...
                            print("\n" + "=" * 80)
                            print("✓ TOKENS SUCCESSFULLY LOADED - YOUR WORK IS PRESERVED!")
                            print("=" * 80)
                            print(f"✓ Loaded {len(example.tokens):,} tokens from disk")
                            print("✓ Your existing tokens are safe and will NOT be overwritten")
                            print("=" * 80)
                            use_loaded_data = True
                        except Exception as e:
    ...
    try:
        ...
        if needs_data:
            if not use_loaded_data:
                ...
                if 'a' in features_to_run or 'c' in features_to_run:
                    ...
                    # Step 4: [PRIORITY] STORE IN WEAVIATE FIRST (CRITICAL STEP)
                    # This happens IMMEDIATELY after tokenization/embedding
                    # Weaviate is cloud-based and reliable - ensures data safety
                    # All subsequent operations will use this stored data
                    if 'c' in features_to_run:
                        example.store_tokens_weaviate_first()
            else:
                # Data loaded from disk
                print("\n" + "=" * 80)
                print("✓ USING YOUR EXISTING DATA - NO DATA WILL BE OVERWRITTEN")
                print("=" * 80)
                print(f"✓ Using {len(example.tokens):,} loaded tokens")
                if example.embeddings is not None and len(example.embeddings) > 0:
                    print(f"✓ Using {len(example.embeddings):,} loaded embeddings")
                print("✓ All existing data is preserved and safe")
                print("=" * 80)
                ...
                if example.embeddings is None or len(example.embeddings) == 0:
                    if example.tokens and len(example.tokens) > 0:
                        ...
                        if 'a' in features_to_run:
                            ...
                            # Optionally store in Weaviate FIRST (priority)
                            if 'c' in features_to_run:
                                try:
                                    store_choice = safe_input("\n[INFO] Store tokens in Weaviate? (y/n) [default: y]: ", default="y", non_interactive_default="y")
                                    if store_choice == "y":
                                        example.store_tokens_weaviate_first()
                                except (KeyboardInterrupt, EOFError):
                                    pass
        ...
        # [FINAL] PHASE 8: FINAL STORAGE - Store in other vector stores (FAISS, ChromaDB) at the END
        # This happens AFTER all other operations are complete
        # WHY AT THE END?
        # - Weaviate already has everything (stored in Phase 2, line 2758)
        # - All search/analysis operations already completed using Weaviate
        # - If FAISS/ChromaDB fail here, no data is lost (Weaviate has it all)
        # - This is "bonus" storage for faster local search, not critical
        if 'c' in features_to_run and (example.faiss_store or example.chroma_store):
            print("\n" + "=" * 80)
            print("FINAL STEP: STORING IN OTHER VECTOR STORES")
            print("=" * 80)
            print("[INFO] Weaviate storage completed earlier. Now storing in FAISS/ChromaDB...")
            print("[INFO] Note: If FAISS/ChromaDB fail, no problem - Weaviate already has all data.")
            example.store_tokens_other_stores()
