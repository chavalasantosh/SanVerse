...
class SanTOKSequenceOptimizer:
    ...
    def count_parameters(self) -> int:
        ...
        for block in self.blocks:
            # Pattern matcher
            count += block.pattern_matcher.W_q.size
            count += block.pattern_matcher.W_k.size
            count += block.pattern_matcher.W_v.size
            count += block.pattern_matcher.W_o.size
            # Processor
            count += block.processor.W1.size
            count += block.processor.b1.size
            count += block.processor.W2.size
            count += block.processor.b2.size
            # Layer norms
