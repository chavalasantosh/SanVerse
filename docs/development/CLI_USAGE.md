# SanTOK CLI - Complete Usage Guide

## One Clean Entry Point for Everything

The `santok_cli.py` file is your single entry point for ALL SanTOK operations.
...
# Tokenize file
python santok_cli.py tokenize --file data.txt --method word --output tokens.json

# Tokenize from URL
python santok_cli.py tokenize --url https://example.com/text.txt --method word
...
```bash
# Train on text
python santok_cli.py train --text "Your corpus here..." --model-path model.pkl

# Train on file
python santok_cli.py train --file corpus.txt --model-path model.pkl --epochs 20

# Train from URL
python santok_cli.py train --url https://example.com/corpus.txt --model-path model.pkl

# Use enhanced trainer
python santok_cli.py train --file corpus.txt --enhanced --epochs 10

# Options:
#   --text TEXT           Training text
#   --file PATH           Training file path
#   --url URL             Training URL
#   --model-path PATH     Model output path (default: santok_model.pkl)
...
python santok_cli.py embed --file data.txt --model-path model.pkl --output embeddings.npy

# Use different strategy
python santok_cli.py embed --text "Hello" --strategy feature_based

# Options:
#   --text TEXT        Input text
#   --file PATH        Input file path
#   --model-path PATH  Trained model path (default: santok_model.pkl)
...
```bash
# Tokenize a file
python santok_cli.py tokenize --file document.txt --method word --output tokens.json
```
...
python santok_cli.py train --file corpus.txt --model-path my_model.pkl --epochs 15

# Step 2: Generate embeddings
python santok_cli.py embed --file new_text.txt --model-path my_model.pkl --output embeddings.npy
```
...
One file (`santok_cli.py`) handles everything. Clean, simple, complete.
