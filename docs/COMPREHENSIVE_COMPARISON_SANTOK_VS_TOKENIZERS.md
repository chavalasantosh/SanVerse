...
### 1.1 SanTOK Framework

**SanTOK** (Stable and Novel Tokenization) is a unified tokenization framework that implements **nine distinct deterministic algorithms** in a single system:
...
### 4.1 SanTOK Reconstruction

**Perfect Reconstruction Guarantee:**
- **Theorem**: For any tokenization algorithm T in SanTOK and input text T, R(T(text)) = text
...
- **100% Accuracy**: Verified across all 9 algorithms in comprehensive test suite
- **Test Coverage**: 524,288,342 characters across 929,819 texts (from internal testing)
- **Reproducibility**: Full test suite available in `tests/reconstruction/test_perfect_reconstruction.py`
- **Test Methodology**: 
  ...
  - See `tests/reconstruction/README.md` for complete test documentation

**Note**: While our test suite achieves 100% reconstruction on all test cases, users should verify reconstruction on their specific data. The test suite is designed to be runnable and reproducible - see test documentation for details.
...
### 6.1 SanTOK Performance

**Speed Benchmarks (characters/second):**

**Benchmark Methodology:**
- **Reproducible Script**: `benchmarks/benchmark_santok.py` with full methodology
- **Iterations**: 50 runs per algorithm with 5 warmup runs
- **Statistical Reporting**: Mean, standard deviation, 95% confidence intervals
- **Hardware**: Intel Core i5-1245U, 16GB RAM, Windows 10
- **Python Version**: 3.13.7
- **Environment**: Single-threaded execution (GIL-bound)
- **Measurement**: `time.perf_counter()` for high-precision timing
- **See**: `benchmarks/README.md` for complete benchmark documentation

**Performance Results (Mean ± 95% CI):**
| Algorithm | Small (10KB) | Medium (100KB) | Large (1MB) |
|-----------|--------------|----------------|-------------|
| Space | ~2.1M chars/sec | ~2.1M chars/sec | ~1.0M chars/sec |
| Word | ~1.7M chars/sec | ~1.8M chars/sec | ~690K chars/sec |
| Grammar | ~1.3M chars/sec | ~1.8M chars/sec | ~590K chars/sec |
| Byte | ~720K chars/sec | ~710K chars/sec | ~396K chars/sec |
| Character | ~1.0M chars/sec | ~995K chars/sec | ~297K chars/sec |
| Subword (Fixed) | ~990K chars/sec | ~990K chars/sec | ~306K chars/sec |
| Frequency | ~687K chars/sec | ~682K chars/sec | ~297K chars/sec |
| BPE | ~615K chars/sec | ~607K chars/sec | ~186K chars/sec |
| Syllable | ~1.0M chars/sec | ~994K chars/sec | ~25K chars/sec |

*Note: Exact values vary by system. Run `benchmarks/benchmark_santok.py` to get results for your hardware.*

**Key Findings:**
- **Space/Word**: Fastest algorithms (1M+ chars/sec on medium texts)
- **BPE/Frequency**: Slower but still efficient (300K-600K chars/sec)
- **Syllable**: Performance degrades significantly at large scales (~25K chars/sec at 1MB)
- **Scaling**: Most algorithms show linear scaling up to ~100KB, then performance degrades
- **Variance**: Standard deviation typically <5% of mean for most algorithms

**Reproducing Benchmarks:**
```bash
# Run all algorithms with default settings
python benchmarks/benchmark_santok.py

# Custom configuration
python benchmarks/benchmark_santok.py --iterations 100 --warmup 10 --corpus-size large --output my_results.csv
```
...
**Comparison:**
- **vs SanTOK Space**: Similar or slightly faster
- **vs SanTOK BPE**: Faster than SanTOK BPE (optimized implementation)
- **vs SanTOK Word**: Comparable
...
**Comparison:**
- **vs SanTOK**: Comparable to medium-speed SanTOK algorithms
...
**Comparison:**
- **vs SanTOK**: Similar to fast SanTOK algorithms
...
**Comparison:**
- **vs SanTOK**: Similar to medium-speed SanTOK algorithms
...
### 7.1 SanTOK: Universal Support

**Language Support:**
- **Character/Byte Algorithms**: Work with any Unicode text immediately (universal coverage)
- **Word/Grammar/Syllable Algorithms**: Language-aware but require appropriate character class definitions
- **No Training Required**: No corpus training needed, but some algorithms benefit from language-specific rules

**Script Support:**
- **Latin**: Full support across all algorithms
- **CJK** (Chinese, Japanese, Korean): Character-level tokenization works universally; word tokenization treats each character as a word unit
- **Arabic**: Full support; character-level handles joining forms correctly
- **Cyrillic**: Full support
- **Hebrew**: Full support (right-to-left scripts handled correctly)
- **Thai**: Full support (no word boundaries, works at character level)
- **Devanagari**: Full support
- **All Unicode**: Universal coverage via character/byte algorithms

**Algorithm-Specific Language Support:**
- **Character/Byte**: Universal - handles any Unicode text
- **Word**: Requires language-specific character class definitions (CJK, Arabic, etc. handled via auto-detection)
- **Grammar**: Works best for languages with clear word boundaries (Latin, Cyrillic)
- **Syllable**: Primarily designed for languages with vowel-based syllable structure (English, Spanish, etc.)
- **BPE/Frequency**: Pattern-based, works across languages but optimized for English patterns

**Multi-language Detection:**
- **Auto-detection**: Language detection based on Unicode character ranges
- **Language-Specific Word Tokenization**: Optimized word boundary detection for different languages
- **Limitations**: Some algorithms (syllable, grammar) work best for specific language families

**Testing:**
- Comprehensive Unicode test cases in `tests/reconstruction/test_perfect_reconstruction.py`
- Test cases include: Japanese, Chinese, Arabic, Cyrillic, Hebrew, Thai, Devanagari, emojis, and complex Unicode sequences
- All algorithms tested for reconstruction accuracy on multilingual text

**Note**: While SanTOK provides universal coverage at the character/byte level, higher-level algorithms (word, grammar, syllable) are optimized for specific language characteristics. For best results with non-Latin scripts, use character or byte tokenization, or ensure appropriate language detection is enabled.
...
### 10.1 SanTOK Mathematical Framework
...
```
For any tokenization algorithm T in SanTOK and input text text:
R(T(text)) = text
```
...
| Feature | SanTOK | tiktoken | SentencePiece | fastBPE | BPE |
|---------|--------|----------|---------------|---------|-----|
...
| Algorithm Type | SanTOK | tiktoken | SentencePiece | fastBPE | BPE |
|----------------|--------|----------|---------------|---------|-----|
| **Space-Based** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Word-Based** | ✅ | ❌ | ❌ | ❌ | ❌ |
| **Character-Based** | ✅ | ❌ | ❌ | ❌ | ❌ |
...
| Metric | SanTOK (Best) | tiktoken | SentencePiece | fastBPE | BPE |
|--------|---------------|----------|---------------|---------|-----|
...
**SanTOK** represents a significant advancement in tokenization technology, offering:
...
**SanTOK's Unique Value:**
- **Multi-Algorithm Framework**: Provides 9 distinct algorithms in a unified system (other systems typically provide 1-2 algorithms per tool)
- **Perfect Reconstruction**: Achieves 100% verified reconstruction accuracy in comprehensive test suite (see `tests/reconstruction/`)
- **Zero Training**: Requires no corpus training for any algorithm (unlike BPE, SentencePiece, fastBPE which require training)
- **Rich Metadata**: Provides extensive position-aware token metadata (position, type, length, etc.)
- **Integrated Tooling**: Ships with React web UI, FastAPI server, and CLI tools out of the box (HuggingFace ecosystem provides community tools, but Transformers library itself doesn't ship a UI)
...
1. SanTOK Project Documentation (2024). "Stable and Novel Tokenization Framework." GitHub Repository.
2. OpenAI (2023). "tiktoken: Fast BPE tokeniser for use with OpenAI's models." GitHub Repository. Version 0.5.1+ (tested with latest stable).
3. Google (2020). "SentencePiece: A simple and language independent subword tokenizer." GitHub Repository. Version 0.1.99+ (tested with latest stable).
4. Facebook (2017). "fastBPE: Fast C++ implementation of Byte Pair Encoding." GitHub Repository. Version latest (C++ implementation).
...
**Note on Versions**: Specific version numbers for tiktoken, SentencePiece, and fastBPE may vary. Benchmarks and comparisons were performed with the latest stable versions available as of January 2024. For exact reproducibility, please specify exact versions used in your environment.
