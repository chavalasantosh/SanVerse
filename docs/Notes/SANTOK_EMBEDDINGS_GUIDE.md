# SanTOK Embeddings Guide - Complete Reference
...
SanTOK embeddings convert tokens into high-dimensional vector representations that capture semantic and structural information. These embeddings enable similarity search, clustering, and other downstream tasks.
...
SanTOK supports **4 different embedding strategies**:

1. **feature_based**: Deterministic embedding from SanTOK token features
...
|---|----------|-------------|---------------|------------------|----------|
| 1 | **feature_based** | Based on SanTOK token features | Yes | No | Fast, reproducible embeddings |
| 2 | **semantic** | Learned semantic relationships | Yes (after training) | Yes | Semantic similarity search |
| 3 | **hybrid** | Text + SanTOK features | No | No (uses pretrained) | Best semantic understanding |
...
Feature-based embeddings generate deterministic vectors from SanTOK's token features (UID, frontend digit, backend number, etc.). This strategy requires no training and produces reproducible embeddings.
...
- **Feature-Rich**: Captures all SanTOK token properties
...
2. **Tokenization**: Convert text to tokens using SanTOK
...
Hybrid embeddings combine text-based embeddings (from sentence-transformers) with SanTOK feature-based embeddings. This combines the best of both worlds: semantic understanding from text models and structural information from SanTOK features.
...
- **Structure Aware**: Includes SanTOK features
...
This guide covers SanTOK's embedding system:
...
*This guide provides a complete reference for understanding and implementing SanTOK embeddings. All strategies have been verified against the actual implementation.*
