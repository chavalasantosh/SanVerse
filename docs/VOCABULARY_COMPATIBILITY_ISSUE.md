# Vocabulary Compatibility Issue: SanTOK ↔ Pretrained Models
...
1. **SanTOK generates its own token IDs:**
...
3. **The Mismatch:**
   - SanTOK's token ID `12345` ≠ BERT's token ID `12345`
   - Feeding SanTOK IDs directly to a pretrained model will produce **garbage embeddings**
   - The model interprets SanTOK's ID `12345` as whatever token BERT has at index 12345 (likely a completely different token)
...
### Solution 1: Use SanTOK Tokenization + Model Vocabulary Mapping (Recommended)

**Use SanTOK to tokenize, then map token strings to model vocabulary IDs.**
...
model_ready = converter.convert_santok_result(santok_result, tokenizer_type="word")
...
**Pros:**
- ✅ Leverages SanTOK's superior tokenization
- ✅ Compatible with any pretrained model
- ✅ Preserves SanTOK metadata (frontend digits, backend numbers)
- ✅ No model retraining required

**Cons:**
- ⚠️ Model tokenizer may split tokens differently (subword tokenization)
- ⚠️ Alignment between SanTOK tokens and model tokens may not be 1:1

### Solution 2: Train New Model with SanTOK from Day One

**Build a new model using SanTOK's vocabulary system from scratch.**

```python
# This requires training a new model
# 1. Create vocabulary from SanTOK tokenization of training corpus
# 2. Initialize embedding layer with SanTOK vocabulary size
...
**Replace the embedding layer of a pretrained model with one aligned to SanTOK's vocabulary.**
...
1. **SanTOK Tokenization**: SanTOK tokenizes text into token strings
...
5. **Alignment**: Approximate mapping between SanTOK tokens and model tokens

### Important Notes

⚠️ **Subword Tokenization Mismatch**: If the model uses subword tokenization (BPE, WordPiece, SentencePiece), a single SanTOK token may map to multiple model tokens.
...
✅ **SanTOK Metadata Preserved**: SanTOK's frontend digits, backend numbers, and UIDs are preserved in the conversion result.

## Best Practices

1. **Use SanTOK's tokenization strategies that align with your model:**
...
2. **Always check the mapping:**
   ...
   print(result["token_mapping"])  # Check SanTOK → Model token alignment
   ```

3. **Preserve SanTOK metadata:**
   ```python
   # Access SanTOK's original data
   frontend_digits = result["santok_frontend_digits"]
   backend_scaled = result["santok_backend_scaled"]
   ```

4. **For new projects: Consider training with SanTOK from scratch** if you have the resources.

## FAQ

**Q: Can I use SanTOK IDs directly with pretrained models?**  
A: No. SanTOK IDs are not compatible with pretrained model vocabularies. Use the vocabulary adapter.

**Q: Will I lose SanTOK's precision by using the adapter?**  
A: You keep SanTOK's tokenization precision, but the model may split tokens differently during subword tokenization.

**Q: Can I fine-tune a model to use SanTOK's vocabulary?**  
...
A: Match SanTOK's strategy to your model's tokenization type (BPE, WordPiece, etc.) for best alignment.

## Conclusion

SanTOK's tokenization is superior, but **vocabulary compatibility is essential** for using pretrained models. The vocabulary adapter provides a bridge between SanTOK's tokenization and model vocabularies, allowing you to leverage both:

- ✅ SanTOK's superior tokenization
- ✅ Pretrained models' powerful embeddings and representations

For new projects with sufficient resources, training from scratch with SanTOK's vocabulary system provides the best long-term solution.
