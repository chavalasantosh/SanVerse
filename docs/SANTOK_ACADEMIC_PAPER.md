# SanTOK: A Unified Multi-Algorithm Tokenization Framework with Guaranteed Perfect Reconstruction
# SanTOK: All you need is attention Tokenization comes first.

**Authors:** SanTOK Development Team\*  
**Affiliation:** SanTOK Project  
**Date:** January 2024  
**Version:** v1.0
...
This paper introduces SanTOK, a unified tokenization framework that addresses these limitations through the following contributions:
...
**Our Approach**: SanTOK addresses these limitations by providing a unified framework with nine deterministic algorithms, guaranteed perfect reconstruction, and zero training requirements.
...
Each token in SanTOK contains complete metadata for reconstruction:
...
**Theorem 1** (Perfect Reconstruction): For any tokenization algorithm T in SanTOK and input text text, R(T(text)) = text.
...
Where n is the input text length. All algorithms maintain linear time complexity, making SanTOK scalable to large texts.
...
The SanTOK framework consists of four main subsystems:
...
*Figure 1: SanTOK System Architecture. The framework processes input text through a tokenization layer, generates position-aware tokens, and provides reconstruction and validation capabilities.*

### 4.3 API and Tooling

SanTOK provides multiple interfaces:
...
- Performance benchmarks: `benchmarks/benchmark_santok.py`
...
We conducted performance benchmarks using the benchmark suite (`benchmarks/benchmark_santok.py`) with:
...
We compared SanTOK against existing tokenization frameworks:

| Framework | Algorithms | Reconstruction | Training | Speed Range | Languages |
|-----------|------------|----------------|----------|-------------|-----------|
| **SanTOK** | **9** | **100%** | **None** | **25K-2.1M** | **Universal** |
...
We evaluated SanTOK's language support across multiple scripts:
...
**Model Integration**: SanTOK algorithms are not yet integrated into major language models, limiting direct comparison with model-specific tokenizers. Integration would require:
- Vocabulary mapping from SanTOK tokens to model embeddings
- Custom embedding layers aligned with SanTOK tokenization
- Fine-tuning of models with SanTOK-based tokenization
...
**Community Adoption**: As a research framework, SanTOK has limited community adoption compared to established tools like SentencePiece or tiktoken. This limits:
- Third-party integrations
- Community-contributed improvements
- Real-world production usage data

**Unicode Normalization**: SanTOK does not currently apply Unicode normalization (NFC/NFKC). This may affect reconstruction when input text uses different normalization forms. Future work will address this limitation.

**Scalability**: While SanTOK handles large texts (100GB+), performance degrades at very large scales. Parallel processing support is planned for future versions.
...
We presented SanTOK, a unified tokenization framework that implements nine distinct deterministic algorithms with mathematically guaranteed perfect reconstruction. Through comprehensive evaluation, we demonstrated 100% reconstruction accuracy across all algorithms and competitive performance compared to existing solutions. SanTOK eliminates training dependencies, enabling immediate deployment across any language or domain while providing superior accuracy and algorithmic diversity.

The framework represents a significant advancement in tokenization technology, addressing critical limitations of existing systems through perfect reconstruction, zero training requirements, and comprehensive algorithmic diversity. Future work will focus on model integration, performance optimization, and community adoption to establish SanTOK as a standard tool for tokenization research and production applications.

---

## Acknowledgments

We thank the open-source community for inspiration and feedback. This work was supported by the SanTOK project development team.
...
- Source: `benchmarks/benchmark_santok.py`
...
python benchmarks/benchmark_santok.py --iterations 100 --warmup 10 --corpus-size large --output results.csv

# Benchmark specific algorithms
python benchmarks/benchmark_santok.py --algorithms word,char,byte
```

### A.3 Code Availability

SanTOK source code, test suites, and benchmarks are available in the project repository:
...
# Verify installation
python -c "from src.core.core_tokenizer import tokenize_text; print('SanTOK installed successfully')"
```
...
python benchmarks/benchmark_santok.py --output experiments_$(date +%Y%m%d).csv
...
**Correspondence**: SanTOK Development Team
